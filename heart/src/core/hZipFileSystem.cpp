...
namespace Heart
{
    ...
    void hZipFileSystem::Initialise( const hChar* zipFile )
    {
        ...
        hUint syspathsize=hdGetSystemPathSize(zipFile);
        hChar* syspath=(hChar*)hAlloca(syspathsize);
        hdGetSystemPath(zipFile, syspath, syspathsize);
        hChar* folder = hStrRChr(syspath, '/');
        if (!folder) {
            folder = hStrRChr(syspath, '/');
        }
        if (folder) {
            basePathLen_=(hUint)((hUintptr_t)folder-(hUintptr_t)syspath)+1;
        } else {
            basePathLen_=0;
        }
    ...
    {
        if (mode != FILEMODE_READ || !IsOpen()) {
            return hNullptr;
        }

        hUint syspathsize=hdGetSystemPathSize(filename);
        if (syspathsize < basePathLen_) {
            return hNullptr;
        }
        hChar* syspath=(hChar*)hAlloca(syspathsize);
        hdGetSystemPath(filename, syspath, syspathsize);
        unzGoToFirstFile(zipFileHandle_);
        int fileFound = unzLocateFile(zipFileHandle_, syspath+basePathLen_, 1);
