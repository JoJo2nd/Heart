...
static mstate init_user_mstate(char* tbase, size_t tsize) {
 ...
 mspace HEART_API create_mspace(size_t capacity, int locked) {
  mstate m = 0;
  size_t msize;
  ensure_initialization();
 ...
 mspace HEART_API create_mspace_with_base(void* base, size_t capacity, int locked) {
  mstate m = 0;
  size_t msize;
  ensure_initialization();
  msize = pad_request(sizeof(struct malloc_state));
 ...
 int HEART_API mspace_track_large_chunks(mspace msp, int enable) {
  int ret = 0;
 ...
 size_t HEART_API destroy_mspace(mspace msp) {
  size_t freed = 0;
  mstate ms = (mstate)msp;
  if (ok_magic(ms)) {
...
*/
 ...
 void* HEART_API mspace_malloc(mspace msp, size_t bytes) {
  mstate ms = (mstate)msp;
  if (!ok_magic(ms)) {
    USAGE_ERROR_ACTION(ms,ms);
    return 0;
 ...
 void HEART_API mspace_free(mspace msp, void* mem) {
  if (mem != 0) {
    mchunkptr p  = mem2chunk(mem);
#if FOOTERS
    mstate fm = get_mstate_for(p);
 ...
 void* HEART_API mspace_calloc(mspace msp, size_t n_elements, size_t elem_size) {
  void* mem;
  size_t req = 0;
  mstate ms = (mstate)msp;
  if (!ok_magic(ms)) {
 ...
 void* HEART_API mspace_realloc(mspace msp, void* oldmem, size_t bytes) {
  if (oldmem == 0)
 ...
 void* HEART_API mspace_memalign(mspace msp, size_t alignment, size_t bytes) {
  mstate ms = (mstate)msp;
  if (!ok_magic(ms)) {
    USAGE_ERROR_ACTION(ms,ms);
    return 0;
 ...
 void** HEART_API mspace_independent_calloc(mspace msp, size_t n_elements,
 ...
 void** HEART_API mspace_independent_comalloc(mspace msp, size_t n_elements,
 ...
 int HEART_API mspace_trim(mspace msp, size_t pad) {
  int result = 0;
  mstate ms = (mstate)msp;
  if (ok_magic(ms)) {
    if (!PREACTION(ms)) {
 ...
 void HEART_API mspace_malloc_stats(mspace msp, size_t* peak, size_t* curr, size_t* allocd ) {
  mstate ms = (mstate)msp;
  if (ok_magic(ms)) {
 ...
 size_t HEART_API mspace_footprint(mspace msp) {
  size_t result = 0;
  mstate ms = (mstate)msp;
  if (ok_magic(ms)) {
    result = ms->footprint;
 ...
 size_t HEART_API mspace_max_footprint(mspace msp) {
  size_t result = 0;
  mstate ms = (mstate)msp;
  if (ok_magic(ms)) {
...
#if !NO_MALLINFO
 struct mallinfo HEART_API mspace_mallinfo(mspace msp) {
  mstate ms = (mstate)msp;
  if (!ok_magic(ms)) {
    USAGE_ERROR_ACTION(ms,ms);
  }
 ...
 size_t HEART_API mspace_usable_size(void* mem) {
  if (mem != 0) {
    mchunkptr p = mem2chunk(mem);
    if (is_inuse(p))
      return chunksize(p) - overhead_for(p);
 ...
 size_t HEART_API mspace_allocate_size( void* mem )
{
	return mspace_usable_size(mem);
}

 int HEART_API mspace_valid_pointer(mspace msp,void* mem)
{
#if defined (FOOTERS)
    mchunkptr p  = mem2chunk(mem);
    mstate fm = get_mstate_for(p);
 ...
 int HEART_API mspace_mallopt(int param_number, int value) {
  return change_mparam(param_number, value);
}
