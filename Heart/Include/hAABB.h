...
namespace Heart
{
	...
	public:
		...
		static hAABB				rotate( const hAABB& a, const hMatrix& m );
		static hAABB				MatrixMult( const hAABB& a, const hMatrix& m );
		static void				    expandBy( hAABB& a, const hVec3& point );
...
// 		return false;
        ...
        if ( hVec128AllGreaterEqual( hVec3Func::length( a.c_ - point ).Get128(), a.r_.Get128() ) )
            return hFalse;
        return hTrue;
	}
...
// 
// 		return hTrue;
        if ( hVec128AllLessEqual( hVec128Abs( ( a.c_ - b.c_ ).Get128() ), ( a.r_ + b.r_ ).Get128() ) )
            return hTrue;

        return hFalse;
	}

	inline hAABB hAABB::rotate( const hAABB& a, const hMatrix& m )
	{
...
//   		b.r[2] = abs(m.m13)*a.r[0] + abs(m.m23)*a.r[1] + abs(m.m33)*a.r[2];
	...
	inline hAABB hAABB::MatrixMult( const hAABB& a, const hMatrix& m )
	{
...
// 		out.r[ 2 ] = (MaxZ - MinZ) / 2;
	...
	inline void hAABB::expandBy( hAABB& a, const hVec3& point )
	{
// 		hVec3 t[3];
// 		t[ 2 ] = point;
// 
