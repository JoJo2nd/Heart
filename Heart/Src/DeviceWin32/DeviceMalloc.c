...
void mspace_malloc_stats(mspace msp, size_t* peak, size_t* curr, size_t* allocd ) {
  mstate ms = (mstate)msp;
  if (ok_magic(ms)) {
...
size_t mspace_allocate_size( void* mem )
{
// 	if (mem != 0) 
// 	{
// 		mchunkptr p = mem2chunk(mem);
...
int mspace_valid_pointer(mspace msp,void* mem)
{
	mstate ms = (mstate)msp;
	char* c = (char*)mem;
	if (mem != 0)
